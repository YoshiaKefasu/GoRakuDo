...
class PostsManager {
    ...
    renderPosts(posts) {
        ...
        posts.forEach((post, index) => {
            ...
            setTimeout(() => {
                this.checkFileExists(this.generateGitHubUrl(post.filename)).then(exists => {
        ...
        try {
            ...
            const hasErrors = await Promise.all(
                this.postsConfig.posts.map(post => 
                    this.checkFileExists(this.generateGitHubUrl(post.filename)).then(exists => !exists)
                )
...
class PostViewer {
    ...
    async loadPost() {
        if (!this.loader.markdownUrl) {
            this.showError('Parameter URL tidak ditemukan. Gunakan format: post.html?url=https://raw.githubusercontent.com/...');
            return;
        }
        ...
        try {
            const markdownContent = await this.loader.loadFromGitHubUrl(this.loader.markdownUrl);
            const extractedTitle = this.loader.extractTitleFromMarkdown(markdownContent);
            const postTitle = this.loader.title !== 'Dokumentasi GoRakuDo' ? this.loader.title : extractedTitle;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (isPostsPage) {
        ...
        console.log('📖 Initializing Post Viewer...');
