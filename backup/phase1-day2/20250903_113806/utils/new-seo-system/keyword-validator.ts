...
export class NewSEOKeywordValidator {
  ...
  validateAll(keywords: {
    ...
    if (keywords.primary) {
      this.validateKeywordArray(keywords.primary, 'PRIMARY' as KeywordType, result);
    }
    
    if (keywords.longTail) {
      this.validateKeywordArray(keywords.longTail, 'LONG_TAIL' as KeywordType, result);
    }
    
    if (keywords.article) {
      this.validateKeywordArray(keywords.article, 'SECONDARY' as KeywordType, result);
    }
    
    if (keywords.category) {
      this.validateKeywordArray(keywords.category, 'SECONDARY' as KeywordType, result);
    }
  ...
  private generateOptimizationSuggestions(result: ValidationResult): void {
    ...
    if (result.optimizedKeywords.length > 15) {
      result.suggestions.push({
        ...
        message: 'キーワードが多すぎる場合、関連性の高いものに絞り込むことをお勧めします',
        field: 'keywords',
        priority: 'MEDIUM',
        expectedBenefit: 'SEO品質の向上'
      });
