...
export class QualityEvaluator {
  ...
  evaluateLength(text) {
    ...
    if (
      length >= this.qualityThresholds.minLength &&
      length <= this.qualityThresholds.maxLength
    ) {
      return 100;
    }
  ...
  getQualityStats(scores) {
    ...
    const totalReadability = scores.reduce(
      (sum, score) => sum + score.readability,
      0
    );
    const totalRelevance = scores.reduce(
      (sum, score) => sum + score.relevance,
      0
    );
    const acceptableCount = scores.filter(score =>
      this.isQualityAcceptable(score)
    ).length;
