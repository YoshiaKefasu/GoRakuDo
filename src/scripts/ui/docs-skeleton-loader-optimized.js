...
class DocsSkeletonLoader {
  ...
  showSkeleton(containerId, config) {
    try {
      ...
      if (!container) {
        console.warn(`⚠�E�EContainer not found: ${containerId}`);
        return;
      }
      ...
      for (let i = 0; i < count; i++) {
        switch (type) {
          ...
          case 'relationship-card':
            ...
            break;
          default:
            console.warn(`⚠�E�EUnknown skeleton type: ${type}`);
            break;
        }
      }
...
if (typeof window !== 'undefined') {
  ...
  function initializeDocsSkeletonLoader() {
    console.log('🦴 Initializing Docs Skeleton Loader...');

    try {
      window.docsSkeletonLoader = new DocsSkeletonLoader();
      console.log('✁ESkeleton loading utility loaded successfully');
    } catch (error) {
      console.error('Error initializing docs skeleton loader:', error);
    }
  }
