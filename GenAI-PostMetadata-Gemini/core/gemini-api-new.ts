...
export class GeminiAIServiceNew {
    ...
    try {
      const response = await this.ai!.models.generateContent({
        ...
        config: {
          ...
          topK: 64,
          thinkingConfig: {
            thinkingBudget: 0, // Disables thinking for faster meta description generation
          },
        },
      });
    ...
    try {
      const response = await this.ai!.models.generateContent({
        ...
        config: {
          ...
          topK: 50,
          thinkingConfig: {
            thinkingBudget: 0, // Disables thinking for faster content recommendations
          },
        },
      });
  ...
  async generateKeywords(content: string, title: string): Promise<string[]> {
    ...
    const prompt = `Write 5-8 relevant keywords from this content for SEO purposes. Return only the keywords as a JSON array of strings.
...
Content: ${content.substring(0, 1000)}...`;
    ...
    try {
      const response = await this.ai!.models.generateContent({
        ...
        config: {
          ...
          topK: 30,
          thinkingConfig: {
            thinkingBudget: 0, // Disables thinking for faster keyword generation
          },
        },
      });
  ...
  private buildMetaDescriptionPrompt(request: MetaDescriptionRequest): string {
    ...
    return `Write an SEO-optimized meta description in ${language} for this content. 
Requirements:
